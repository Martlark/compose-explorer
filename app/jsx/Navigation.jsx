...
export const Navigation = (props) => {
  ...
  useEffect(() => {
    ...
    context.api
      .projects(context.serverId)
      .then((result) => setProjects(result))
      .fail((xhr, textStatus, errorThrown) =>
        context.setErrorMessage(
          `Error getting projects: ${xhr.responseText} - ${errorThrown}`
        )
      );
  }, [context.serverId]);
  ...
  const projectLinks = (
    <NavDropdown title="Projects" id="projects-dropdown">
      {projects.map((project) => {
        return (
          <NavDropdown.Item>
            <Link to={`/server/${context.serverId}/project/${project.name}`}>
              {project.name}
            </Link>
          </NavDropdown.Item>
        );
    ...
    <Navbar expand="lg">
      ...
      <Navbar.Collapse id="navbar">
        <Nav className="mr-auto">
          <Nav.Link>
            <Link to={`/server/${context.serverId}`}>
