...
export default function AuditAdmin() {
  ...
  return (
    <div>
      <h2>Audit Records</h2>
      <Button style={{ marginRight: "1em" }} onClick={clickDeleteSelected}>
        <span className="material-icons">delete</span>Delete selected
      </Button>
      <Button style={{ marginRight: "1em" }} onClick={clickRefresh}>
      ...
      <table className={"table"}>
        ...
        <tbody>
          {records.map((record) => (
            <AuditRecord record={record} clickSelected={clickSelected} />
          ))}
        </tbody>
      </table>
    </div>
