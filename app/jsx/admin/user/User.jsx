...
function GroupMembership({ user, groups, authService, context }) {
  ...
  function onChange(evt, action) {
    ...
    switch (action.action) {
      case "select-option":
        groupService
          .add_user(null, { group_id, user_id })
          .then((result) => context.setMessage(result));
        break;
      case "remove-value":
        groupService
          .remove_user(null, { group_id, user_id })
          .then((result) => context.setMessage(result));
        break;
    }
  }
...
export default function User({ user, groups, authService, getUsers }) {
  ...
  function clickUpdateSetPassword(evt) {
    ...
    authService
      ...
      .fail((xhr, textStatus, errorThrown) =>
        context.setErrorMessage(
          `Error set password: ${xhr.responseText} - ${errorThrown}`
        )
      );
  }
  ...
  function renderPassword() {
    return (
      <form onSubmit={clickUpdateSetPassword}>
        <input type="hidden" name="id" defaultValue={user.id} />
  ...
  function renderConfirmDelete() {
    return (
      <form onSubmit={clickSubmitDelete}>
        <input type="hidden" name="id" defaultValue={user.id} />
        <Button style={buttonStyle} variant="danger" size="sm" type={"submit"}>
          Confirm {isLdapUser ? "clear" : "delete"}
        </Button>
        <Button
          style={buttonStyle}
          size="sm"
  ...
  function updateUserType(newType) {
    authService
      .put(authService.urlJoin("user", user.id), { user_type: newType })
      .then((result) => {
        setUserType(newType);
        context.setMessage(
          `${result.message} ${result.item.email} type: ${result.item.user_type}`
        );
      })
      .fail((xhr, textStatus, errorThrown) =>
        context.setErrorMessage(
  ...
  function renderUserTypes() {
    ...
    return (
      ...
      >
        {authService.user_types.map((opt) => (
          <option>{opt}</option>
        ))}
      </Form.Control>
    );
  ...
  function renderEmail() {
    ...
    return (
      <span title="Edit" style={{ cursor: "pointer" }} onClick={clickEdit}>
        {user.email}
      </span>
    );
  }
  ...
  return (
    <tr>
      <td>{mode === "edit" ? renderEdit() : renderEmail()}</td>
      <td>{renderUserTypes()}</td>
      <td>
