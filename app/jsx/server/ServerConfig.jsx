...
function ServerGroupMembershipItem({
  group,
  server,
  setMessage,
  setMembershipChanged,
}) {
  const [isMember, setIsMember] = useState(
    server.group_membership.map((gm) => gm.id).includes(group.id)
  );
...
function ServerGroupMembership({ server_id, setMessage }) {
  ...
  useEffect(() => {
    serverService
      .json(serverService.urlJoin("server", server_id))
      .then((result) => {
        setServer(result);
      });
    ...
    <div>
      ...
      <table className="table">
        {server &&
          groups.map((group) => {
            return (
              <ServerGroupMembershipItem
                setMembershipChanged={setMembershipChanged}
                group={group}
                server={server}
      ...
      {!hasMembership && (
        <h4>Group membership is required to allow non admin user access</h4>
      )}
    </div>
  );
}
...
function ServerEditShowItem(props) {
  ...
  function clickTestConnection(evt) {
    ...
    api
      .testConnection(formData.name, formData.port, formData.credentials)
  ...
  if (props.edit) {
    ...
    const form = (
      <Form onSubmit={(evt) => clickSubmit(evt)}>
...
export function ServerConfig(props) {
    ...
    return (
      <span>
        ...
        <InlineConfirmButton
          ...
          textValues={["Delete", "Confirm", "Deleting"]}
          showTimer
  ...
  function renderSummary() {
    ...
    return (
      <span title="number of containers">{item?.summary?.containers}</span>
    );
  }
