...
function DirectoryEntry(props) {
  ...
  const editButton = (
    <a
      href={"#"}
      className={"btn"}
      onClick={(evt) => clickEditSelected(evt)}
  ...
  function linkToEdit() {
    const fileName = join(
      props.directoryPath,
      entry.linked_file_name || entry.file_name
    );
    const encodedFileName = encodeURIComponent(fileName);
    return (
      <Link
        to={`/server/${props.id}/container_file_edit/${props.name}?filename=${encodedFileName}`}
        target="_blank"
      >
        <span className="material-icons">edit</span>
      </Link>
  ...
  function renderFilename() {
    if (entry.dir_type === "d") {
      return (
        <a href={"#"} onClick={(evt) => clickDirectory(evt, entry.file_name)}>
          {entry.file_name}
        </a>
      );
    }
...
export default function Directory(props) {
  ...
  function clickDeleteSelected(evt) {
    ...
    const cmd = `(cd ${directoryPath} && rm ${selected
  ...
  function clickDownloadSelected(evt) {
    const selected = directoryEntries.filter(
      (dir) => dir.selected && dir.dir_type !== "d"
    );
    selected.forEach((dir) => {
      const filename = join(
        directoryPath,
        dir.linked_file_name || dir.file_name
      );
      return context.api
  ...
  function getDirectory(pwd) {
    setDirectoryGetting(true);
    return context.api
      .proxyGet(`/container/${id}/ls/`, { name: name, pwd: pwd })
      .then((result) => {
        setPwd(result.pwd);
  ...
  function renderActions() {
    ...
    const upButton =
      directoryPath !== "/" ? (
        <a
          href={"#"}
          className={"btn"}
          onClick={(evt) => clickUpDirectory(evt)}
          title={"Up one level of directory"}
        >
    ...
    const deleteButton = (
      <a
        href={"#"}
        className={"btn"}
        onClick={(evt) => clickDeleteSelected(evt)}
    ...
    const downloadButton = (
      <a
        href={"#"}
        className={"btn"}
        onClick={(evt) => clickDownloadSelected(evt)}
    ...
    const refreshButton = (
      <a
        href={"#"}
        className={"btn"}
        onClick={(evt) => clickDirectory(evt, pwd)}
        title={"Refresh"}
      >
        <span className="material-icons">cached</span>
      </a>
    );
    const getting = directoryGetting ? (
    ...
    return (
      <div>
        ...
        {directoryLinks.map((d) => (
          <a
            href={"#"}
            onClick={(evt) => clickDirectory(evt, d.cwd)}
            title={d.cwd}
          >
            {d.dir}&nbsp;/&nbsp;
          </a>
